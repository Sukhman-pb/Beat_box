...
static snd_pcm_t *handle;    //data type for the PCCM handle
...
typedef struct {
	...
	int location;
	//add more stuff depending upon the implementation 
} playbackSound_t;
static playbackSound_t soundBites[MAX_SOUND_BITES]; //to be used in the queue and fillPlaybackBuffer functions
// Playback threading
...
void AudioMixer_init(void)
{
	...
	for (int i = 0; i < MAX_SOUND_BITES; i++) {
		soundBites[i].pSound = NULL;  // Ensure no sound is playing initially
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	assert(pSound->pData);
	//assert(pSound->numSamples < MAX_SOUND_BITES);//making sure the sound bite is not too large

	//using concurrent access to store the sound bites
	for (int i = 0; i < MAX_SOUND_BITES;i++){
		if (soundBites[i].pSound == NULL){
			pthread_mutex_lock(&audioMutex);
			//checking if the sound bite has already been filled
			if(soundBites[i].pSound != NULL){
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	//now we will access sound bites array
	for(int i =0; i < MAX_SOUND_BITES; i++){
		if(soundBites[i].pSound == NULL){
			continue;// try next slot if this one is empty
		}
		else{ //NOT NULL
			pthread_mutex_lock(&audioMutex);
			count++;
			//checking if it has been changes by some other thread while we were waiting on the lock
			if(soundBites[i].pSound == NULL){
				pthread_mutex_unlock(&audioMutex);
				continue;
			}
			...
			int remainingSamples = soundBites[i].pSound->numSamples - offset;
			int samplesToCopy = (remainingSamples < size) ? remainingSamples : size;
			...
			soundBites[i].location += samplesToCopy;
			//checking if the sound bite has been played
	...
	//     int offset = soundBites[i].location;
	//     int remaining = soundBites[i].pSound->numSamples - offset;
	//     int samplesToCopy = (remaining < size) ? remaining : size;
	
	//     for (int j = 0; j < samplesToCopy; j++) {
	//         int mixedSample = buff[j] + soundBites[i].pSound->pData[offset + j];
